...
# 0 = rest, 1 = close, 2 = open, 3 = right, 4 = left, 5 = point (if you add it)
...
def _channel_variance_feature(batch_windows: np.ndarray) -> np.ndarray:
    """
    Returns [N,1] = variance of per-channel RMS across the 8 channels.
    High when activation is concentrated in a few channels, low when spread evenly.
    batch_windows: [N, L, 8, 1]
    """
    W = batch_windows[..., 0]                       # [N, L, 8]
    rms = np.sqrt(np.mean(W**2, axis=1))            # [N, 8]
    var = np.var(rms, axis=1, keepdims=True)        # [N, 1]
    return var.astype(np.float32)


def _channel_sparsity_feature(batch_windows: np.ndarray, eps: float = 1e-8) -> np.ndarray:
    """
    Returns [N,1] = max_rms / mean_rms across channels.
    High when one channel dominates (localized activation),
    low when energy is spread evenly.
    batch_windows: [N, L, 8, 1]
    """
    W = batch_windows[..., 0]                       # [N, L, 8]
    rms = np.sqrt(np.mean(W**2, axis=1))            # [N, 8]
    max_r = np.max(rms, axis=1, keepdims=True)      # [N, 1]
    mean_r = np.mean(rms, axis=1, keepdims=True)    # [N, 1]
    ratio = max_r / (mean_r + eps)                  # [N, 1]
...
def fine_tune_model(
    ...
    """
    Fine-tunes the MLP on CNN embeddings + 4 scalar EMG features:
      - side asymmetry  (right vs left)
      - magnitude       (overall EMG strength)
      - channel variance (spread vs localized)
      - sparsity ratio   (max/mean RMS)

    recorded_data:  list/array of windows [L,8,1]
    recorded_labels: list/array of ints in {0..K-1}
    """
    model = load_model(
        feature_extractor_path,
    ...
    v_train = _channel_variance_feature(X_train)     # [N,1]
    v_val   = _channel_variance_feature(X_val)       # [N,1]

    s_train = _channel_sparsity_feature(X_train)     # [N,1]
    s_val   = _channel_sparsity_feature(X_val)       # [N,1]

    feats_train_plus = np.hstack([feats_train, a_train, m_train, v_train, s_train])  # [N, D+4]
    feats_val_plus   = np.hstack([feats_val,   a_val,   m_val,   v_val,   s_val])    # [N, D+4]
